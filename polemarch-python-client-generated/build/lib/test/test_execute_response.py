# coding: utf-8

"""
    Polemarch

     ### Polemarch is ansible based service for orchestration infrastructure.  * [Documentation](http://polemarch.readthedocs.io/) * [Issue Tracker](https://gitlab.com/vstconsulting/polemarch/issues) * [Source Code](https://gitlab.com/vstconsulting/polemarch)    # noqa: E501

    OpenAPI spec version: v2
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import unittest

import polemarch_client
from polemarch_client.models.execute_response import ExecuteResponse  # noqa: E501
from polemarch_client.rest import ApiException


class TestExecuteResponse(unittest.TestCase):
    """ExecuteResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testExecuteResponse(self):
        """Test ExecuteResponse"""
        # FIXME: construct object with mandatory attributes with example values
        # model = polemarch_client.models.execute_response.ExecuteResponse()  # noqa: E501
        pass


if __name__ == '__main__':
    unittest.main()
